...
public abstract class GenericDao<T, PK extends Serializable> implements IGenericDao<T, PK>  {
	...
	public <kls> List<? extends kls> list(String sql, Class<?> kls) {
		SQLQuery qrySql = getSession().createSQLQuery( sql ).addEntity(kls);
		return qrySql.list();
	}
...
*/
		...
		SQLQuery qryCount = getSession().createSQLQuery(SqlCount);
		...
		SQLQuery qrySql = getSession().createSQLQuery(Sql).addEntity(entityClass); // MVS kls
